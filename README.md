# 👋 Hi, I'm Patrick Darmawan Rahardjo

💻 Fullstack Developer | 🎨 UI/UX Enthusiast
🚀 Passionate about building scalable web applications and solving real-world problems

---

## 🧠 About Me

Fresh graduate in Computer Science from Bina Nusantara University with hands-on experience in full-stack development and UI/UX design.

I enjoy building modern web applications using **Next.js, Node.js, and TypeScript**, and I’m especially interested in **real-time systems and scalable backend architecture**.

---

## 🛠️ Tech Stack

### 💻 Frontend
<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,html,css,tailwind,bootstrap" />
</p>

### ⚙️ Backend
<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,php" />
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/strapi/strapi-original.svg" height="48" alt="Strapi"/>
</p>

### 🗄️ Database & Backend Services
<p>
  <img src="https://skillicons.dev/icons?i=postgres,mysql" />
  <img src="https://seeklogo.com/images/S/supabase-logo-DCC676FFE2-seeklogo.com.png" height="48" alt="Supabase"/>
  <img src="https://avatars.githubusercontent.com/u/77690634?s=200&v=4" height="48"/>
</p>

### ☁️ Deployment & Hosting
<p>
  <img src="https://skillicons.dev/icons?i=vercel" />
  <img src="https://raw.githubusercontent.com/render-oss/render-docs/main/static/img/logo.svg" height="48" alt="Render"/>
</p>

### 🧪 Tools & Design
<p>
  <img src="https://skillicons.dev/icons?i=postman,git,github,figma" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jira/jira-original.svg" height="48" alt="Jira"/>
  <img src="https://cdn.worldvectorlogo.com/logos/miro-2.svg" height="48"/>
</p>

---

## 💼 Work Experience

### 🏢 PT Ganda Visi Jayatama

**Backend Engineer**
*Using Node.js, TypeScript, Strapi, PostgreSQL*

* Developed and maintained multiple production websites
* Built REST APIs and tested endpoints using Postman
* Managed CMS using Strapi

🌐 Projects:

* https://www.sibs.sch.id/id
* https://asiaharumsedap.com/
* https://abn.farm/

---

### 🏢 PT Berkat Elektrik Sejati Tangguh

**IT Engineer**
*Using Php, Html, Css, Js, Mysql*

* Redesigned company websites (UI/UX) using Figma
* Delivered production-ready company websites
* Maintained and monitored BEST & BESTLIFE websites
* Improved performance and user experience
* Collaborated to optimize workflows

🌐 Projects:

* https://best-life.id/
* https://ptberkat.co.id/

---

## 🚀 Featured Projects

### 🍔 Nganteen — Food Ordering App

* Fullstack web app for campus food ordering
* Real-time ordering system concept
* Built with Next.js, TypeScript, Drizzle, PostgreSQL

👉 Role: UI/UX Designer & Fullstack Developer

---

### 🪑 Carters

* Furniture rental platform with pricing transparency
* Focus on UX structure & user flow

👉 Role: UI/UX Designer

---

### 👁️ Matakita

* App concept for visually impaired assistance via video call
* Volunteer-based support system

👉 Role: UI/UX Designer

---

## 📊 GitHub Stats

![Patrick's GitHub stats](https://github-readme-stats.vercel.app/api?username=PatrickDarmawanR\&show_icons=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=PatrickDarmawanR\&layout=compact)


---

## 📫 Contact Me

* 📧 Email: [pdr242003@gmail.com](mailto:pdr242003@gmail.com)
* 💼 LinkedIn: https://www.linkedin.com/in/patrick-darmawan-rahardjo
* 🎨 Portfolio: https://www.behance.net/patrickdarmawan

---

## ⚡ Let's Build Something Great Together!

I'm open to opportunities in **Frontend, Backend, or Fullstack Development** 🚀
